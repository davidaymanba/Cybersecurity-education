...
<head>
    ...
    <title>{{ $title ?? 'CyberLearn AI' }}</title>
        ...
        <div class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
            ...
            <nav class="hidden items-center gap-5 text-sm text-slate-300 md:flex">
                @auth
                    ...
                    <a href="{{ route('lessons.index') }}" class="hover:text-white">Lessons</a>
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="hover:text-white">Admin</a>
                    @endif
                    <form action="{{ route('logout') }}" method="post">@csrf<button class="hover:text-white">Logout</button></form>
                @else
                    <a href="{{ route('login') }}" class="hover:text-white">Login</a>
                    <a href="{{ route('register') }}" class="rounded-lg bg-cyan-400 px-4 py-2 font-semibold text-slate-950 shadow-lg shadow-cyan-500/20">Register</a>
                @endauth
            </nav>
        </div>
    </header>
